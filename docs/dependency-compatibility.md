...
- Vite manual chunks keep React-bound UI/runtime packages in `vendor-react`. Do not split Radix, lucide-react, sonner, cmdk, vaul, React Router, or React Query into a separate `vendor-ui` chunk; Vercel deployments have produced `Cannot read properties of undefined (reading 'useLayoutEffect')` from a split UI chunk.
